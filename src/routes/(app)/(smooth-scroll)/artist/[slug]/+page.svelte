<script lang="ts">
  ...
  $: allPublications = [
    ...
    const isDuplicate = unique.some(
      (item) =>
        item._createdAt === pub._createdAt ||
        item.slug?.current === pub.slug?.current,
    );
      ...
      {#each siteDocuments.sections as s, index}
          ...
          {#await import('@/components/common/artwork/Artwork.svelte') then Artwork}
            <Artwork.default
              ...
              props={{
                ...
                artworkAtLast: true,
                ctaLink:
                  s.artworkLink?.href || `/artist/${slug.current}/collection`,
        ...
        {:else if s._type === 'artist.publication'}
          <Publication
            ...
            artistName={personalDocuments.name} />
        {/if}
      {/each}
