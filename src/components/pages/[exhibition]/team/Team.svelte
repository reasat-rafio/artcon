<script lang="ts">
  ...
  import type { TeamProps } from '@/lib/types/exhibition-detail.types';
  import type { EmblaCarouselType } from 'embla-carousel';
  import emblaCarouselSvelte from 'embla-carousel-svelte';

  export let props: TeamProps;
  ...
  $: chunksOfMembers = chunkArray(members, slidesNumber);
  $: if (emblaApi) {
...
<section use:parallaxAnimation class="translate-y-[120px]">
  ...
  <div
    ...
    on:emblaInit={onInit}>
    <div class="flex">
      {#each chunksOfMembers as chunk}
        <div
          class="grid flex-[0_0_100%] grid-cols-1 gap-x-[3.56rem] gap-y-[3.12rem] sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3">
          {#each chunk as { image, name, role }}
            <div class="group flex h-fit items-center justify-center gap-x-[1.25rem]">
  ...
  <div class="mt-[2.31rem] flex items-center justify-between">
    ...
    <span
      ...
      {selectedSnap + 1}/{chunksOfMembers.length}
    </span>
  </div>
</section>
